...
import { isOutputNodePath, Material, MaterialNode } from "../../types/material.ts";
...
export default function ExportDialogOutputNodesPanel(props: Props) {
    const allOutputNodes = () => props.material.nodes.filter((x) => isOutputNodePath(x.path));
    ...
    function toggleOutputNode(node: MaterialNode) {
        if (props.selectedNodes.has(node.id)) {
            ...
            if (!props.fileNames.has(node.id)) {
                props.fileNames.set(node.id, props.material.name + "_" + node.label);
            }
