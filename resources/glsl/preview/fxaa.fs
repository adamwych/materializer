...
vec3 FxaaPixelShader(vec2 pos, sampler2D tex, vec2 rcpFrame) {
    ...
    vec2 posP = posN;
    vec2 offNP = horzSpan ? vec2(rcpFrame.x, 0.0f) : vec2(0.0f, rcpFrame.y);
    float lumaEndN = lumaN;
    float lumaEndP = lumaN;
    bool doneN = false;
    bool doneP = false;
    posN += offNP * vec2(-1.0f, -1.0f);
    posP += offNP * vec2(1.0f, 1.0f);
    ...
    vec3 rgbF = texture(tex, vec2(pos.x + (horzSpan ? 0.0f : subPixelOffset), pos.y + (horzSpan ? subPixelOffset : 0.0f))).xyz;
